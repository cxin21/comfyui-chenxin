...
| `runtime/` | — | v7 本地执行合同：TaskContext、CapabilityReport、workflow fingerprint、ExecutionDraft、hash-bound approval event、approved ExecutionPlan、camera allowlist patch、RunRecord 与 JSON CLI。`plan` 只产未批准 draft；`approve-plan` 才能把展示后的 exact `draft_hash` 绑定到一次 enqueue。 |
