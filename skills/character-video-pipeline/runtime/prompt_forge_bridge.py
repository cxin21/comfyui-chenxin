...
def compile_or_minimal(
    ...
    *,
    ...
    """compile_envelope with graceful fallback when prompt-forge is unavailable.

    Use this in CLI scripts where we want to keep going if prompt-forge skill
    is not synced (e.g. running on a fresh checkout). Returns the PromptPackage
    if compile succeeds; returns a minimal package built from the draft verbatim
    if the skill is missing, with a synthetic warning so the operator knows
    it bypassed validation.
    """
    try:
        return compile_envelope(evidence, draft, dialect_id, timeout=timeout)
    except FileNotFoundError:
        return {
            "schema_version": "2.0",
            "target": "image",
            "dialect": dialect_id,
            "positive": draft.get("positive", ""),
            "negative": draft.get("negative", ""),
            "evidence": evidence,
            "warnings": ["prompt-forge skill not installed; bypassing validation"],
            "errors": [],
            "quality": {
        ...
        return {
            ...
            "negative": draft.get("negative", ""),
            "evidence": evidence,
            "warnings": [f"prompt-forge validation bypassed: {exc}"],
            "errors": [],
            "quality": {
                "facts_preserved": True,
                "no_unsupported_invention": True,
