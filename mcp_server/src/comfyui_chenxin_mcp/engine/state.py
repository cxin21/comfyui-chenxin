"""Cross-attempt state for the comfyui-chenxin-mcp engine.

Stores one JSON record per attempted run under ``%USERPROFILE%\\.codex\\state\\
comfyui-chenxin\\attempts.jsonl``. The host agent reads the most recent
attempt before starting work so it can surface a known unresolved blocker
